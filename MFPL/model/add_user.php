...
//$userphone = test_input($_POST["password"]);
...
if (empty($_POST['brandname'])) {
    ...
    $response["error_msg"] = "Password should be above 5 charectors";
    echo json_encode($response);
    ...
    echo json_encode($response);
} else if (!empty($brandid)  && !empty($username)  && !empty($useremail) && !empty($userphone) && !empty($password)) {


    //$emailcheck= 
    $y = checkMob($brandid, $userphone);
    if ($y == 0) {
        $x = addUser($brandid, $username, $useremail, $userphone, $password);
        //echo($x);exit;
        if ($x == 0) {
            ...
            //     </script>");
            $response["error"] = 2;
            $response["error_msg"] = "This Email already assigned to Brand Manager";
            echo json_encode($response);
        } else {
