...
device
    .on('connect', function () {
        // console.log('connect');
        device.subscribe('recipePacket/EPAN/+');
...
device
    .on('message', function (topic, payload) {
        ...
        data = recpack['recipe'];
        if (topic == 'recipePacket/EPAN/'+MACID) {
            //console.log(topic);
            ...
            if (pt == 'EPan') {
                ...
                con.connect(function (err) {
                    if (err) {
                        device.publish(`response/EPAN/${MACID}`, JSON.stringify({ "MACID": MACID, "timestamp": timestamp, "status": "0" }));
                        //throw err;
                    }
                    // console.log("Connected!");
                    //var sql = "INSERT INTO rcpdata( time, SLN, `ptype`, `macid`, `rcptype`, `rcpname`, `rcpstarttime`, `rcpendtime`,`rcpercd`, `finalop`, `rc`,`appname`,`cookingtype`,`timestamp`) VALUES (time, 'SLN', 'PType','MACID','Rcpcry','Rcpnme','rcpstarttime','rcpendtime','Rcpercd','Fnlop',RC,'appName','cooktype','timestamp')";
                    ...
                    con.query(sql1, [values1], function (err, result) {
                        ...
                        // console.log(values);
                        if (err) {
                            device.publish(`response/EPAN/${MACID}`, JSON.stringify({ "SLN": MACID, "timestamp": timestamp, "status": "0" }));
                            //throw err;
                        } else {
                            device.publish(`response/EPAN/${MACID}`, JSON.stringify({ "SLN": MACID, "timestamp": timestamp, "status": "1" }));
                        }
                        // if(i==0){
